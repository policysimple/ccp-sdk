<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: blockchain/network/v1alpha1/network_api.proto

namespace Blockchain\Network\V1alpha1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Generated from protobuf message <code>blockchain.network.v1alpha1.AddPeerToOrganizationRequest</code>
 */
class AddPeerToOrganizationRequest extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>string org_domain = 1 [json_name = "orgDomain"];</code>
     */
    protected $org_domain = '';
    /**
     * Generated from protobuf field <code>string admin_password = 2 [json_name = "adminPassword"];</code>
     */
    protected $admin_password = '';
    /**
     * Generated from protobuf field <code>string admin_user = 3 [json_name = "adminUser"];</code>
     */
    protected $admin_user = '';
    /**
     * Generated from protobuf field <code>string network_name = 4 [json_name = "networkName"];</code>
     */
    protected $network_name = '';
    /**
     * Generated from protobuf field <code>string network_id = 5 [json_name = "networkId"];</code>
     */
    protected $network_id = '';

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string $org_domain
     *     @type string $admin_password
     *     @type string $admin_user
     *     @type string $network_name
     *     @type string $network_id
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Blockchain\Network\V1Alpha1\NetworkApi::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>string org_domain = 1 [json_name = "orgDomain"];</code>
     * @return string
     */
    public function getOrgDomain()
    {
        return $this->org_domain;
    }

    /**
     * Generated from protobuf field <code>string org_domain = 1 [json_name = "orgDomain"];</code>
     * @param string $var
     * @return $this
     */
    public function setOrgDomain($var)
    {
        GPBUtil::checkString($var, True);
        $this->org_domain = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string admin_password = 2 [json_name = "adminPassword"];</code>
     * @return string
     */
    public function getAdminPassword()
    {
        return $this->admin_password;
    }

    /**
     * Generated from protobuf field <code>string admin_password = 2 [json_name = "adminPassword"];</code>
     * @param string $var
     * @return $this
     */
    public function setAdminPassword($var)
    {
        GPBUtil::checkString($var, True);
        $this->admin_password = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string admin_user = 3 [json_name = "adminUser"];</code>
     * @return string
     */
    public function getAdminUser()
    {
        return $this->admin_user;
    }

    /**
     * Generated from protobuf field <code>string admin_user = 3 [json_name = "adminUser"];</code>
     * @param string $var
     * @return $this
     */
    public function setAdminUser($var)
    {
        GPBUtil::checkString($var, True);
        $this->admin_user = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string network_name = 4 [json_name = "networkName"];</code>
     * @return string
     */
    public function getNetworkName()
    {
        return $this->network_name;
    }

    /**
     * Generated from protobuf field <code>string network_name = 4 [json_name = "networkName"];</code>
     * @param string $var
     * @return $this
     */
    public function setNetworkName($var)
    {
        GPBUtil::checkString($var, True);
        $this->network_name = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string network_id = 5 [json_name = "networkId"];</code>
     * @return string
     */
    public function getNetworkId()
    {
        return $this->network_id;
    }

    /**
     * Generated from protobuf field <code>string network_id = 5 [json_name = "networkId"];</code>
     * @param string $var
     * @return $this
     */
    public function setNetworkId($var)
    {
        GPBUtil::checkString($var, True);
        $this->network_id = $var;

        return $this;
    }

}

